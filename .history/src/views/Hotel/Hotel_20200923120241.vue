                ...
                .then((res) => {
                  ...
                  res.data.map((item) => {
                    let marker = new AMap.Marker({
                      position: new AMap.LngLat(item.location.latitude,item.location.longitude), // 经纬度对象，也可以是经纬度构成的一维数组[116.39, 39.9]
                      title: item.name,
                    });
                  });
                  let marker = new AMap.Marker({
                    position: new AMap.LngLat(is.location), // 经纬度对象，也可以是经纬度构成的一维数组[116.39, 39.9]
                  });
