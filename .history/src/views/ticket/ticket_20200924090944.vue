      ...
      <div class="leftall">
        <div style="width: 100%; margin-top: 15px" class="topleft">
          <div style="display: flex">
            <div>单程:</div>
            <div>{{ destCity }}-{{ departCity }}{{ departDate }}</div>
          </div>
          <div style="display: flex">
            <!-- 起飞机场 -->
            <a-select
        ...
        <div style="margin-top: 7px">
          筛选:
          <a-button type="info" shape="round" @click="undo">撤销</a-button>
        </div>
        <!-- 表格标题 -->
        ...
        <!-- 表格内容 -->
        <div class="for" style="margin-top: 7px" v-if="flights.length > 0">
          <div
            v-for="(item, index) in flights.slice(
        ...
        <div class="historyall">
          <div class="his">
            ...
            <div class="hittory">
              ...
              <div>
                <a-button style="background-color: orange; color: white"
                  >选择</a-button
                >
              </div>
            </div>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
      ...
      api
        ...
        .then((res: any) => {
          ...
          res.flights.map((item: any) => {
            item.show = false;
            item.startTime = Number(item.dep_time.slice(0, 2));
          });
    ...
    //起飞时间过滤
    let handleChange3 = (value: value): void => {
      console.log(value);
      data.value2 = value.time;
      data.flights = data.flights1.filter((item: item1) => {
        return value.from <= item.startTime && item.startTime < value.to;
      });
      data.total = data.flights.length;
    };
    //飞机型号筛选
    ...
    //购买机票
    let butticket = (ite: any): void => {
      router.push({
        path: "/buyticket",
        query: {
          ota_id: ite.ota_id,
          seat_xid: ite.seat_xid,
        },
      });
    };
    //撤销
