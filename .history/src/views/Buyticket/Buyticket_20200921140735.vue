...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    let removeDomain = (item: any): void => {
      let index = data.dynamicValidateForm.domains.indexOf(item);
      if (index !== -1) {
        data.dynamicValidateForm.domains.splice(index, 1);
      }
    };
    let addDomain = (): void => {
      console.log(data.dynamicValidateForm.domains);

      data.dynamicValidateForm.domains.push({
        value: "",
        key: Date.now(),
      });
    };
