<template>
  <a-carousel arrows>
    ...
    <div v-for="(item,index) in figure" :key="index" class="forimg">
      ...
      >
        <div>
          <div v-for="(item,index) in input" :key="index" style="display:flex">
            <div class="Trapezoid">{{item.name}}</div>
            <div class="bottonLeft"></div>
          </div>
        </div>
...
.bottonLeft {
  width: 0;
  height: 0;
  border-bottom: 50px solid rgb(212, 209, 209);
