    ...
    <div class="strategy">
      ...
      <div class="right">
        ...
        <div v-for="(item,index) in article" :key="index">
          ...
          >{{item.summary}}</div>
          <div style="display:flex;width:100%;flex-wrap: wrap;border:1px solid red">
            <div v-for="(ite,ind) in item.images" :key="ind" style="width:30%;">
              <img
                :src="`${ite}`"
                v-if="(ind+1)/4===1 || ind===0"
                style="width:100%;height:180px;object-fit:cover;margin-left:10px"
              />
              <img :src="`${ite}`" v-else style="width:100%;height:200px;object-fit:cover;" />
            </div>
          </div>
        </div>
      </div>
