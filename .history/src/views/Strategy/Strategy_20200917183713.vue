    ...
    <div class="strategy">
      ...
      <div class="right">
        ...
        >
          ...
          >{{item.summary}}</div>
          <div v-if="item.images.length>0" style="  display: grid;                                      //设置为网格布局					   
  grid-template-rows: 1fr 1fr 1fr;display:flex;width:100%;flex-wrap:wrap;">
            <div v-for="(ite,ind) in item.images" :key="ind" style="width:32%;flex:1;">
              <img :src="`${ite}`" style="width:200px;height:200px;object-fit:cover;" />
            </div>
          </div>
          <!-- 地点，作者，观看次数，点赞次数 -->
