...
export default {
    ...
    hotels({ id, city, price_in, scenic, name_contains, hotellevel, hoteltype, hotelbrand, hotelasset, enterTime, person, _sort, _limit, _start }: {
        ...
        return service.get(`/hotels?id=''&city=${city}&price_in=''&scenic=''&name_contains=''&hotellevel=''&hoteltype=${hoteltype}&hotelbrand=${hotelbrand}&hotelasset=${hotelasset}&enterTime=${enterTime}&person=${person}&_sort=${_sort}&_limit=${_limit}&_start=${_start}`)
    },
    options() {
