...
export default {
    ...
    //酒店详情
    // hotels({ id, city, price_in, scenic, name_contains, hotellevel, hoteltype, hotelbrand, hotelasset, enterTime, person, _sort, _limit, _start }: {
    //     id?: number, city: number, price_in?: number, scenic?: number, name_contains?: string, hotellevel?: number, hoteltype?: number, hotelbrand?: number, hotelasset?: number, enterTime?: Date, person?: number, _sort?: string, _limit?: number, _start?: number
    // }) {
    //     if (city) {
    //         return service.get(`/hotels?city=${city}`)
    //     } //else { return service.get(`/hotels?id=${id}&city=${city}&price_in=${price_in}&scenic=${scenic}&name_contains=${name_contains}&hotellevel=${hotellevel}&hoteltype=${hoteltype}&hotelbrand=${hotelbrand}&hotelasset=${hotelasset}&enterTime=${enterTime}&person=${person}&_sort=${_sort}&_limit=${_limit}&_start=${_start}`) }
    // },
    // hotel({ id, city, price_in, scenic, name_contains, hotellevel, hoteltype, hotelbrand, hotelasset, enterTime, person, _sort, _limit, _start }: {
    //     id?: number, city: number
    // }) {
    //         return service.get(`/hotels?city=${city}`)},
    options() {
